...
def system_prompt_contact_type() -> str:
    ...
    return (
        ...
        "Definitions:\n"
        "- Action Requested: Customer is asking for an action (cancel/modify/resend/update/fix).\n"
        "- Information Requested: Customer is asking for information/status/policy/how-to; not claiming a failure.\n"
...
def system_prompt_DETAILED_SUMMARY() -> str:
    return (
        "You are an expert summarizer for customer support transcripts.\n"
        "Task: Summarize the full conversation in natural language.\n"
        "Rules:\n"
        "- Include all important details from the conversation (do not filter to only key topics).\n"
        "- Capture both customer and agent statements, including requests, explanations, decisions, and next steps.\n"
        "- Keep it clear and readable as 1–2 short paragraphs.\n"
        "- Base the summary ONLY on statements in the transcript; do not infer or speculate.\n"
        "- Do NOT include personal identifiable information (PII).\n"
        "- Do NOT include timestamps, speaker tags, or verbatim quotations longer than a short phrase.\n"
        "- If the transcript lacks sufficient information to summarize, set DETAILED_SUMMARY exactly to:\n"
